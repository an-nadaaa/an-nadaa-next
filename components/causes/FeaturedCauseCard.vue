<template>
  ...
  >
    ...
    <div
      :class="`relative mx-auto lg:max-w-none h-full ${
        index !== 2 ? 'lg:pt-20' : ''
      }`"
    >
      <!-- Testimonial card-->
      <div
        class="relative flex flex-col justify-end h-full pt-64 pb-10 overflow-hidden shadow-xl rounded-2xl"
      >
        <img
        ...
        />
        <div
          class="absolute inset-0 bg-primary-600 opacity-20 mix-blend-multiply"
        />
        <div
          class="absolute inset-0 opacity-60 bg-gradient-to-t from-primary-700 via-primary-700"
        />
        <div class="relative px-8 pb-10">
          ...
          <div
            :class="`mt-3 text-xl font-semibold text-white  ${
              index === 0 ? 'lg:text-5xl' : 'lg:text-3xl'
            }`"
          >
          ...
          <section class="mt-8">
            ...
            <div
              v-else
              class="inline-flex items-center my-3 text-yellow-800 bg-yellow-100 px-2.5 py-0.5 rounded-md"
            >
        ...
        indefinite duration (always open for donations) or an indefinite cost eg: Zakat"
            ...
            <footer class="mt-4">
              ...
              >
                ...
                <span> of </span>
                <span>
                  {{ formateAmount(cause.attributes.dynamicZone[0].goal) }}
                  Raised
                </span>
              </p>
              <div
                v-else-if="cause?.attributes.dynamicZone[0].donation"
                class="font-semibold text-primary-50"
              >
                <span
...
const percentage = computed(() => {
  return props.cause?.attributes.dynamicZone[0].goal
    ? (props.cause.attributes.dynamicZone[0].raised * 100) /
        props.cause.attributes.dynamicZone[0].goal
    : 0
})
</script>
