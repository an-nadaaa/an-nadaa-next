...
const pk = ref(process.env.NODE_ENV === 'production' ? process.env.STRIPE_PK_PROD : process.env.STRIPE_PK_DEV)
...
const { data: cause } = await useAsyncData('cause', () =>
  fetch(`${STRAPI_API}/causes/${route.params.id}?locale=${useI18n().locale}&${causeQuery}`, {
    headers: {
...
async function donate() {
  if (amount.value >= 1) {
    loading.value = true
    await fetch(
      `${process.env.functionBaseUrl}/create-checkout-session?locale=${useI18n().locale.value}&amount=${
        amount.value * 100
      ...
      .then((session) => {
        ...
        return (checkoutRef.value as any).redirectToCheckout()
      })
  }
}
  ...
  meta: [
    {
      hid: 'description',
      name: 'description',
      content: `${cause.value.attributes.title} campaign was launched to ${
        cause.value.attributes.dynamicZone[0].goal
      }, and their state is : ${cause.value.attributes.closed ? 'Closed' : 'Opened'} now.${
        cause.value.attributes.closed ? '' : 'Build your hereafter today by contributing to this campaign'
      } `,
    },
  ],
})
