<template>
  <div class="min-h-full mx-auto lg:mx-24">
    <section class="py-10">
      <!-- Page header -->
      <div
        class="px-4 mx-auto sm:px-6 md:flex md:items-center md:justify-between md:space-x-5 lg:px-8"
      >
          ...
          <div>
            ...
            <p class="text-sm font-medium text-gray-500">
              ...
              <time datetime="2020-08-25">{{
                formatDate(cause.attributes.createdAt)
              }}</time>
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div
            ...
            v-if="
              cause.attributes.dynamicZone[0].__component ===
                'causes.project' && cause.attributes.dynamicZone[0].donation
            "
          >
            <span
      ...
      <div
        class="grid grid-cols-1 gap-6 mx-auto mt-8 sm:px-6 lg:grid-flow-col-dense lg:grid-cols-3"
      >
        <aside
          aria-labelledby="donation-section"
          class="lg:col-start-3 lg:col-span-1"
        >
          <div
            class="px-4 py-5 bg-white shadow-md sm:rounded-lg sm:px-6 lg:sticky lg:top-0"
          >
            <h2
              id="donation-section"
            ...
            <span
              :class="`inline-flex items-center px-2.5 py-0.5 rounded-md text-md mb-3 font-medium ${
                cause.attributes.closed
                  ? 'bg-gary-100 text-gray-800'
                  : 'bg-green-100 text-green-800'
              }`"
            >
              <Component
                class="-ml-0.5 mr-1.5 w-5 h-5"
                :is="cause.attributes.closed ? 'LockIcon' : 'LockOpenIcon'"
              />
            ...
            <div
              v-if="
                cause.attributes.dynamicZone[0].goal &&
                cause.attributes.dynamicZone[0].goal !== 0
              "
            ...
            <ul class="my-6 space-y-6">
              ...
              <div>
                ...
                <ul role="list" class="mt-3 space-y-3">
                  <li
                    v-for="(impact, i) in cause.attributes.dynamicZone[0]
                      .impact"
                    :key="i"
            ...
            <ClientOnly placeholder="Loading...">
              <form
                action="#"
                class="flex-col hidden mt-12 lg:flex sm:mx-auto sm:max-w-lg"
              ...
              <div
                v-if="!cause.attributes.closed"
                class="justify-center hidden w-full lg:flex"
              >
                <button
            ...
            <div class="overflow-hidden bg-white shadow sm:rounded-lg">
              <div id="cause-cover">
                <img
                  ...
                  v-if="
                    cause.attributes.cover.data.attributes.url &&
                    !cause.attributes.video
                  "
                ...
                <VideoPlayer
                  v-else
                  :showPlayer="false"
                  :videoLocation="cause.attributes.video"
                />
              </div>
              <div class="px-4 py-5 border-t border-gray-200 sm:px-6">
                ...
                <div
                  ...
                  v-html="cause.attributes.description"
                ></div>
              </div>
              <div class="p-6">
                <span
          ...
          <section aria-labelledby="cause-information">
            <div class="bg-white shadow sm:rounded-lg sm:overflow-hidden">
              <div class="divide-y divide-gray-200">
                ...
                <div
                  class="px-4 py-6 prose max-w-none sm:px-6 lg:prose-xl"
                  v-html="body"
                ></div>
              </div>
    ...
    <ClientOnly placeholder="Loading...">
      ...
      <div
        v-if="!cause.attributes.closed"
        class="sticky bottom-0 z-50 flex justify-center w-full bg-white lg:hidden"
      >
        <button
          class="inline-flex px-3 py-1 my-4 rounded-lg bg-primary-100 text-primary-700 hover:bg-primary-700 hover:text-primary-100"
          @click="goTo('/direct')"
...
export default {
  ...
  async asyncData({ $axios, app, params, error }) {
    ...
    const cause = await $axios
      .$get(
        `${STRAPI_API}/causes/${params.id}?locale=${app.i18n.locale}&${causeQuery}`,
        {
          headers: {
  ...
  methods: {
    async donate() {
      if (this.amount >= 1) {
        ...
        await this.$axios
          .$post(
            `${process.env.functionBaseUrl}/create-checkout-session?locale=${
              useI18n().locale.value
            }&amount=${this.amount * 100}&product=${
              this.cause.attributes.product
            }`
          )
          .then((session) => {
            this.loading = false
    ...
    formatDate(dateString) {
      ...
      return (
        new Intl.DateTimeFormat(useI18n().locale.value, {
          dateStyle: "full",
        }).format(date) || ""
      )
    },
  ...
  computed: {
    percentage() {
      return this.cause.attributes.dynamicZone[0].goal
        ? (this.cause.attributes.dynamicZone[0].raised * 100) /
            this.cause.attributes.dynamicZone[0].goal
        : 0
    },
    async body() {
      return sanitizeHtml(await marked.parse(this.cause.attributes.body))
    },
    images() {
        ...
        : [
            {
              ...
              thumb: this.cause.attributes.cover.data.attributes.url.replace(
                "photo",
                "thumbnail_photo"
              ),
            },
      ...
      meta: [
        {
          hid: "description",
          name: "description",
          content: `${cause.title} campaign was launched to ${
            cause.goal
          }, and their state is : ${cause.closed ? "Closed" : "Opened"} now.${
            cause.closed
              ? ""
              : "Build your hereafter today by contributing to this campaign"
          } `,
        },
      ],
    }
